...
xDotToFigure s = let p = parsePrimitives ([],s)
                 in composition $ primitivesToFigure p

data XDPrim = XUnfilledEllipse Double Double Double Double
            | XText Double Double String
...
primitivesToFigure :: [XDPrim] -> [Figure]
primitivesToFigure ((XUnfilledEllipse x y w h):ps) =
  (style newStyle {fill = no} $ translate (x,y) $ scale (w, h) $ circle (0,0) 1.0):(primitivesToFigure ps)
primitivesToFigure ((XText x y s):ps) =
  (style newStyle {fill = yes, fillColor = Just black, stroke = no} $ translate (x,y) $ text s):(primitivesToFigure ps)
